const htmlBegin = 
`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Team Organizational Chart</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.5.0/css/bootstrap.min.css">
  <link rel="stylesheet" href="./style.css">

</head>
<body>
  <header class="container text-center bg-danger">
    <h1>My Team</h1>
  </header>
  <main>
    <div id="team-display" class="container text-center">
      <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-3 justify-content-center">`

// const htmlManager = `<p>Office Number: ${officeNumber}</p>`;
// const htmlEngineer = `<p>GitHub: ${githubId}</p>`;
// const htmlIntern = `<p>School: ${school}</p>`;

const empName = 'John Smith';
const email = 'john.smith@mycompany.com';
const position = 'Manager';
const id = '12345';

const htmlEmployeeCard = 
`<div class="col bg-primary m-2 text-light">
  <h2>${empName}</h2>
  <h3>${position}</h3>
  <div class="bg-light text-dark text-left m-1 p-1">
    <p>id: ${id}</p>
    <p>Email: <a href="mailto:${email}">${email}</a></p>
    <p></p>
  </div>
</div>`

const htmlEnd = 
  `   </div>
    </div>
  </main>
  <!-- link to JavaScript libraries and functionality-->
  <script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
  <script type="text/javascript" src="/script.js"></script>
</body>
</html>`

module.exports = {
  htmlBegin, 
  htmlEmployeeCard, 
  htmlEnd
};